...
module procTopD #(parameter X=8, Y=8,pck_num=12,data_width=256,x_size=$clog2(X),y_size=$clog2(Y),total_width=(x_size+y_size+pck_num+data_width))(
input  wire clk,
input  wire rstn,
//PE interfaces
...
for (x=0;x<X;x=x+1) begin:xs
    for (y=0; y<Y; y=y+1) begin:ys
        ...
        begin: instnce
		Decryptor #(.total_width(total_width),.x_size(x_size),.y_size(y_size),.pck_num(pck_num))pe(
			.clk(clk),
			.rst(rstn),
			.i_data(w_data_pe[(total_width*x)+(total_width*X*y)+:total_width]),
			.i_ready(r_ready_pe[x+X*y]),
